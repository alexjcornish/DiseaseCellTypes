test_add.missing.expression <- function() {
    ...
    # 8) outputs numeric matrix with gene names
    ...
    results$single.mixed <- DiseaseCellTypes:::add.missing.expression(expression$single, genes$mixed, score.missing) # 5
    results$all.all <- DiseaseCellTypes:::add.missing.expression(expression$all, genes$all, score.missing) # 6
    results$all.none <- DiseaseCellTypes:::add.missing.expression(expression$all, genes$none, score.missing) # 7
    results$no.context.mixed <- DiseaseCellTypes:::add.missing.expression(expression$no.context, genes$mixed, score.missing) # 8
